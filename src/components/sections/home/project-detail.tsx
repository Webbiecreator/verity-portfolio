    ...
    <section id="project-detail" className="relative w-full overflow-hidden border-t border-white/[0.08] bg-[#070708] text-white">
      <div className="px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
        ...
        <div className="mt-14 grid grid-cols-1 gap-px border border-white/[0.10] bg-white/[0.10] sm:mt-20 md:grid-cols-3">
          {narrative.map(({ label, body }) => <motion.article key={label} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ duration: 0.55 }} className="bg-[#070708] p-6 sm:p-8"><div className="mb-5 font-mono text-[8px] tracking-[0.22em] text-white/30 uppercase">{label}</div><p className="max-w-sm text-sm font-light leading-relaxed text-white/55">{body}</p></motion.article>)}
        </div>

        <div className="mt-14 flex items-center justify-between border-t border-white/[0.10] pt-7 sm:mt-20">
          <button type="button" className="group flex items-center gap-3 text-left" onClick={() => document.getElementById("projects")?.scrollIntoView({ behavior: "smooth" })}><span className="text-lg transition-transform group-hover:-translate-x-1">←</span><span><span className="block font-mono text-[8px] tracking-[0.18em] text-white/25 uppercase">Previous</span><span className="text-sm text-white/60">Selected work</span></span></button>
          <button type="button" className="group flex items-center gap-3 text-right" onClick={() => setActive(true)}><span><span className="block font-mono text-[8px] tracking-[0.18em] text-white/25 uppercase">Next</span><span className="text-sm text-white/60">About Verity</span></span><span className="text-lg transition-transform group-hover:translate-x-1">→</span></button>
        </div>
      </div>
      ...
      {active && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-5 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Continue to About">
          <div className="w-full max-w-md border border-white/15 bg-[#0b0b0c] p-7 sm:p-9">
            ...
            <p className="mt-4 text-sm leading-relaxed text-white/45">Continue into the studio, process and capabilities.</p>
